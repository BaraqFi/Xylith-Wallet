...
interface HomeChainSelectorSheetProps {
    selectedChain: Chain;
...
export function HomeChainSelectorSheet({
    selectedChain,
    selectedEvmChain,
    onSelectChain,
    tokens = [],
    ...
    // Calculate All Networks Total (EVM Only for this component mostly, but supports all passed tokens)
    const allTotal = tokens.reduce((sum, t) => sum + (t.usdValue || 0), 0);

    // Filter supported chains to EXCLUDE Solana
    const filteredChains = SUPPORTED_CHAINS.filter(c => c.type !== 'Solana');
                ...
                <div className="p-4 grid gap-2 max-h-[70vh] overflow-y-auto">
                    ...
                    {filteredChains.map((chain) => {
                        ...
                        const isSelected =
                            ...
                            (!chain.value || selectedEvmChain === chain.value);
